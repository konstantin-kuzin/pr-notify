...
Инструкции для ИИ-агентов, работающих с репозиторием **PR Notify** — Chrome-расширением (Manifest V3), которое показывает pull request'ы в Azure DevOps: на вкладке **Review** — PR на ревью, Waiting for the author и ниже уже одобренные (APPROVED), на вкладке **My PRs** — собственные активные (включая черновики) и Complete PR; в списках — Policies и конфликты слияния.
...
- **`prState`** — списки PR (Review: `items`, `waitingForAuthorItems` и `approvedItems` / My PRs: `myItems`), ошибки, метки времени обновления.
- **`prUpdateState`** — результат проверки новой версии на GitHub.

### Поток данных

```
background.mjs → ado-api.mjs (REST: review pending+waiting-for-author+approved + my/policies; Complete на My PRs — по запросу) → prState в storage → popup.mjs (вкладки Review/My PRs)
