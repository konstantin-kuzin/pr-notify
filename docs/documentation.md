...
7. Результат сортируется **старые выше** по моменту **последнего обновления** PR: в UI это **`updatedAt`** (см. обогащение ниже).
...
2. Если есть участники групп и базовая дата пуша (`lastCommitAt` или `creationDate`):
   - **`GET .../pullRequests/{id}/threads`**;
   - ищется **`lastGroupCommentAt`** — последний **не system** комментарий участника группы, опубликованный **строго после** базовой даты пуша.

В элементе для UI ([`mapPullRequestToItem`](../ado-api.mjs)):

- **`updatedAt`** = max(`lastGroupCommentAt`, `lastCommitAt`, `createdAt`);
- **`createdAt`** = дата создания PR;
- строка «возраст» в popup считается от **`updatedAt ?? createdAt`** до **`lastCheckedAt`** только в **рабочих часах** (пн–пт, **10:00–18:00**, Europe/Moscow) — [`working-time.mjs`](../working-time.mjs).
...
Срочность по максимальному **рабочему** возрасту среди PR ([`getBadgeUrgencyFromItems`](../working-time.mjs)):
...
Жёлтый уровень (>6 ч) **не меняет** иконку toolbar — только **чип времени** в карточке PR в popup.
...
- под заголовком: автор и **относительное рабочее время** (`N мин` / `H ч M мин`) от `updatedAt ?? createdAt` до `lastCheckedAt` (пн–пт, **10:00–18:00** МСК);
- фрагмент времени — **чип** при **> 6 / > 8 / > 16** рабочих ч (жёлтый / оранжевый / красный); пороги совпадают с [`working-time.mjs`](../working-time.mjs);
...
| `working-time.mjs` | Рабочие часы (МСК), пороги срочности, стили badge |
...
Установка: режим разработчика в `chrome://extensions`, «Загрузить распакованное». Актуальная версия в [`manifest.json`](../manifest.json) (на момент сборки документации — **2.6**).
