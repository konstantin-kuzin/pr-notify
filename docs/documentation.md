...
- **`lastCommitAt`**, **`lastGroupCommentAt`** — ISO-даты последнего пуша source и последнего комментария участника группы (после пуша); попадают в элемент UI и используются для **рабочего времени** (см. ниже).
...
|---------|----------------|
| `lastGroupCommentAt` отсутствует | да — комментария группы после пуша ещё не было |
...
|-------------------|---------|-----------------|
| > 6               | yellow  | только **чип времени** в карточке PR |
| > 8               | orange  | чип времени; **иконка** toolbar |
| > 16              | red     | чип времени; **иконка** toolbar |
