...
7. Результат сортируется **старые выше** по моменту **последнего обновления** PR: в UI это **`updatedAt`** (время последнего пуша source после обогащения; если нет — как **`createdAt`**).
...
- **`updatedAt`** = `lastCommitAt` (пуш source), если есть, иначе дата создания PR;
- **`createdAt`** = дата создания PR;
- строка «возраст» в popup считается от **`updatedAt ?? createdAt`** до **`lastCheckedAt`** только в **рабочих часах** (пн–пт, 9:00–18:00, Europe/Moscow).
...
При ошибке API: **badge** показывает ошибку (иконка `icon-*-error.png`, пустой счётчик), в `prState` пишется ошибка, **предыдущий успешный список в `items` не подменяется** на пустой в ветке catch (сохраняется прошлое состояние кроме полей ошибки/времени — см. код `refreshPullRequests`).

Цвет **badge на иконке** (и счётчика в popup) по максимальному **рабочему** возрасту среди PR: серый (≤6 ч), жёлтый (>6 ч), оранжевый (>8 ч), красный (>16 ч) — см. [`working-time.mjs`](../working-time.mjs).
...
- Верх: заголовок, строка «Последняя проверка», кнопка обновления, **счётчик** с числом PR (цвет как у badge на иконке; скрывается при ошибке).
...
- под заголовком: автор и **относительное рабочее время** (`N мин` / `H ч M мин`) от `updatedAt ?? createdAt` до `lastCheckedAt` (пн–пт, 9:00–18:00 МСК);
- фрагмент времени — **чип** при **> 6 / > 8 / > 16** рабочих ч (жёлтый / оранжевый / красный), те же пороги, что у badge;
